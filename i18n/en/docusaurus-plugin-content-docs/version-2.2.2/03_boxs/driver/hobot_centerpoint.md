...
# Extract filesmkdir -p ~/centerpoint_data
...
# Launch the exampleros2 launch hobot_centerpoint hobot_centerpoint_websocket.launch.py lidar_pre_path:=config/hobot_centerpoint_data
```
...
```shell
# Configure the tros.b Humble environmentsource /opt/tros/humble/setup.bash

if [ -L qat ]; then rm qat; fi
ln -s `ros2 pkg prefix hobot_centerpoint`/lib/hobot_centerpoint/qat/ qat
ln -s ~/centerpoint_data centerpoint_data

# Launch the exampleros2 launch hobot_centerpoint hobot_centerpoint.launch.py
```
