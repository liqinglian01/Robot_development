...
# Launch the launch fileros2 launch mono_mobilesam sam.launch.py 
```
...
Run SAM with fixed-box mode disabled (`sam_is_regular_box:=0`):```shell
ros2 launch mono_mobilesam sam.launch.py sam_is_regular_box:=0
```

In another terminal, publish an AI message topic:```shell
